...
import { Menu, Bell, Search, Filter, ShoppingBag, Box, Briefcase, Truck, CheckCircle2, XCircle, ChevronRight, Home, Package, MoreHorizontal } from 'lucide-react-native';

const orders = [
  { id: '#SH12545', status: 'New', customer: 'Rahul Sharma', amount: '₹4,890', items: '3 Items', date: '7 May 2025 | 10:30 AM', icon: ShoppingBag, color: 'bg-orange-500/20', iconColor: '#f97316', statusColor: 'text-green-500', statusBg: 'bg-green-500/10' },
  { id: '#SH12544', status: 'Confirmed', customer: 'Vivek Kumar', amount: '₹7,189', items: '5 Items', date: '7 May 2025 | 09:15 AM', icon: Box, color: 'bg-blue-500/20', iconColor: '#3b82f6', statusColor: 'text-blue-500', statusBg: 'bg-blue-500/10' },
  ...
  return (
    <View className="flex-1 bg-[#050914]">
      ...
      <View className="flex-row items-center justify-between px-6 pt-16 pb-4">
        ...
        <Text className="text-white text-lg font-semibold">Orders</Text>
      ...
      <View className="px-6 mb-4">
        <ScrollView horizontal showsHorizontalScrollIndicator={false} className="flex-row">
          <View className="mr-6 border-b-2 border-[#FF8C00] pb-2">
            <Text className="text-[#FF8C00] font-semibold text-sm">All (1245)</Text>
          </View>
          <View className="mr-6 pb-2">
            <Text className="text-[#94a3b8] font-medium text-sm">New (298)</Text>
          </View>
          <View className="mr-6 pb-2">
            <Text className="text-[#94a3b8] font-medium text-sm">Processing (447)</Text>
          </View>
          <View className="mr-6 pb-2">
      ...
      <ScrollView className="flex-1 px-6 mb-24" showsVerticalScrollIndicator={false}>
        {orders.map((order, index) => (
          <View key={index} className="bg-[#0e1423] border border-[#1e2942] rounded-2xl p-4 flex-row mb-4">
            <View className={`w-12 h-12 rounded-xl items-center justify-center mr-4 ${order.color}`}>
            ...
            <View className="flex-1">
              <View className="flex-row justify-between items-center mb-1">
                ...
                <View className="flex-row items-center">
                  <View className={`px-2 py-1 rounded-md mr-2 ${order.statusBg}`}>
                    <Text className={`text-[10px] font-bold ${order.statusColor}`}>{order.status}</Text>
                  </View>
              ...
              <View className="flex-row justify-between items-center mb-1">
                ...
                <Text className="text-white font-bold">{order.amount}</Text>
              </View>
