...
import { ArrowLeft, Bell, ShoppingBag, UserPlus, AlertTriangle } from 'lucide-react-native';
...
const NotificationItem = ({ type, title, message, time, isUnread }: any) => {
  ...
  return (
    <TwPressable className={`flex-row p-4 border-b border-[#22252D] ${isUnread ? 'bg-[#15171E]' : ''}`}>
      <TwView className="w-12 h-12 rounded-full items-center justify-center mr-4" style={{ backgroundColor: iconBg }}>
        <Icon color={iconColor} size={20} />
      </TwView>
      <TwView className="flex-1 pr-6">
        <TwView className="flex-row justify-between items-start mb-1">
          <TwText className={`text-base font-bold ${isUnread ? 'text-white' : 'text-gray-300'}`}>{title}</TwText>
        </TwView>
        <TwText className="text-gray-400 text-sm leading-5 mb-2">{message}</TwText>
...
export default function NotificationsScreen() {
  ...
  const notifications = [
    ...
    },
    {
      ...
      message: 'Nike Air Zoom Pegasus 40 is running low on stock (Only 2 left).',
      time: '3 hours ago',
      isUnread: false,
    },
    {
