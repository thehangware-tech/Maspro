...
const CouponCard = ({
  ...
  <TwView className="bg-[#15171E] rounded-2xl p-4 mb-4 border border-[#22252D]">
    <TwView className="flex-row items-center justify-between mb-4 border-b border-[#22252D] pb-4">
      ...
      <TwView
        className={`px-2 py-1 rounded-md ${status === "Active" ? "bg-green-900/30" : "bg-red-900/30"}`}
      >
        <TwText
          className={`text-xs font-bold ${status === "Active" ? "text-green-500" : "text-red-500"}`}
        >
          {status}
        </TwText>
      </TwView>
    ...
    <TwView className="flex-row justify-between items-center px-2 mb-3">
      <TwView className="flex-row items-center">
        ...
        <TwView className="bg-[#22252D] px-2 py-1 rounded border border-[#333742] border-dashed">
          <TwText className="text-[#FF6B00] font-bold text-sm tracking-widest">
            {code}
          </TwText>
        </TwView>
      </TwView>
      <TwView className="flex-row items-center">
        ...
        <TwText className="text-gray-400 text-xs ml-1">
          Till: {validTill}
        </TwText>
      </TwView>
    </TwView>
    ...
    <TwView className="bg-[#0A0D14] rounded-lg p-3 flex-row justify-between">
      <TwView>
        <TwText className="text-gray-500 text-[10px] uppercase mb-1">
          Min Order
        </TwText>
      ...
      <TwView className="items-end">
        ...
        <TwText className="text-white font-medium text-sm">
          {maxDiscount}
        </TwText>
      </TwView>
    </TwView>
