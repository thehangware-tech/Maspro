...
const SettingItem = ({
  ...
  <TwPressable className="flex-row items-center justify-between py-4 border-b border-[#22252D]">
    <TwView className="flex-row items-center flex-1">
      <TwView
        className={`w-10 h-10 rounded-xl bg-[#15171E] items-center justify-center mr-4 border border-[#22252D]`}
      >
        <Icon color={isDestructive ? "#EF4444" : "#6B7280"} size={20} />
      </TwView>
      <TwView className="flex-1 mr-4">
        <TwText
          className={`font-medium text-base ${isDestructive ? "text-red-500" : "text-white"}`}
        >
          {title}
        </TwText>
        {subtitle && (
          <TwText className="text-gray-400 text-sm mt-0.5">{subtitle}</TwText>
        )}
      </TwView>
    </TwView>
    {hasSwitch ? (
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#0A0D14" }}>
      ...
      >
        ...
        <TwView className="mb-8">
          <TwText className="text-gray-500 font-bold text-xs uppercase tracking-widest mb-2">
            App Settings
          </TwText>
