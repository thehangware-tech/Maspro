...
import { ArrowLeft, Calendar, TrendingUp, IndianRupee, ShoppingBag, BarChart2, PieChart } from 'lucide-react-native';
...
const StatCard = ({ title, value, change, isPositive, icon: Icon, color }: any) => (
  <TwView className="bg-[#15171E] rounded-2xl p-4 border border-[#22252D] flex-1 mb-4">
    <TwView className="flex-row items-center justify-between mb-3">
      <TwView className="w-10 h-10 rounded-xl items-center justify-center" style={{ backgroundColor: `${color}20` }}>
        <Icon color={color} size={20} />
      </TwView>
      <TwView className={`px-2 py-1 rounded-md flex-row items-center ${isPositive ? 'bg-green-900/30' : 'bg-red-900/30'}`}>
        <TrendingUp color={isPositive ? '#10B981' : '#EF4444'} size={12} style={{ transform: [{ rotate: isPositive ? '0deg' : '180deg' }] }} />
        <TwText className={`text-[10px] font-bold ml-1 ${isPositive ? 'text-green-500' : 'text-red-500'}`}>{change}</TwText>
      </TwView>
    </TwView>
...
const CategoryRow = ({ name, percentage, amount, color }: any) => (
  <TwView className="flex-row items-center justify-between mb-4">
    <TwView className="flex-row items-center">
      <TwView className="w-3 h-3 rounded-full mr-3" style={{ backgroundColor: color }} />
    ...
    <TwView className="flex-row items-center w-1/2">
      <TwView className="flex-1 bg-[#22252D] h-2 rounded-full mr-3 overflow-hidden">
        <TwView className="h-full rounded-full" style={{ width: `${percentage}%`, backgroundColor: color }} />
      </TwView>
      <TwText className="text-gray-400 text-xs w-16 text-right">{amount}</TwText>
    </TwView>
  </TwView>
);
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#0A0D14' }}>
      ...
      <TwView className="flex-row items-center justify-between px-6 py-4">
        ...
        <TwText className="text-white text-lg font-bold">Reports & Analytics</TwText>
        <TwPressable>
      ...
      <ScrollView contentContainerStyle={{ padding: 24 }} showsVerticalScrollIndicator={false}>
        ...
        <TwView className="flex-row bg-[#15171E] rounded-xl p-1 mb-6 border border-[#22252D]">
          {['Today', 'Week', 'Month', 'Year'].map((period, index) => (
            ...
            >
              <TwText className={`text-xs font-bold ${index === 2 ? 'text-white' : 'text-gray-400'}`}>{period}</TwText>
            </TwPressable>
          ))}
        </TwView>
        ...
        <TwView className="bg-[#15171E] rounded-2xl p-4 mb-6 border border-[#22252D]">
          <TwView className="flex-row items-center justify-between mb-4">
            <TwText className="text-white font-bold text-base">Revenue Overview</TwText>
            <TwText className="text-[#FF6B00] text-xs font-bold underline">Detailed Report</TwText>
          </TwView>
          ...
          <TwView className="flex-row justify-between mt-2 px-1">
            {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(day => (
              <TwText key={day} className="text-gray-500 text-[10px] w-8 text-center">{day}</TwText>
            ))}
          </TwView>
        </TwView>
        ...
        {/* Category Breakdown */}
        <TwView className="bg-[#15171E] rounded-2xl p-4 mb-6 border border-[#22252D]">
          <TwText className="text-white font-bold text-base mb-6">Sales by Category</TwText>
          <CategoryRow name="Cricket Gear" percentage={45} amount="₹1.1L" color="#FF6B00" />
          <CategoryRow name="Footwear" percentage={30} amount="₹74K" color="#3B82F6" />
          <CategoryRow name="Apparel" percentage={15} amount="₹37K" color="#10B981" />
          <CategoryRow name="Accessories" percentage={10} amount="₹24K" color="#F59E0B" />
        </TwView>
