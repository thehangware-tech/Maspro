...
import { View, ScrollView, StatusBar, TextInput, Platform, KeyboardAvoidingView } from 'react-native';
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#0A0D14' }}>
      ...
      <TwView className="flex-row items-center px-6 py-4 border-b border-[#22252D]">
        ...
        <TwText className="text-white text-lg font-bold flex-1">Adjust Stock</TwText>
      </TwView>
      ...
      >
        <ScrollView contentContainerStyle={{ padding: 24, paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
          ...
          <TwView className="bg-[#15171E] rounded-xl border border-[#22252D] p-4 mb-6">
            <TwText className="text-white font-bold text-base mb-2">SS Master 1000 Cricket Bat</TwText>
            <TwText className="text-gray-400 text-sm">SKU: SSM1000</TwText>
          </TwView>

          {/* Current Stock */}
          <TwView className="items-center mb-8">
            <TwText className="text-gray-400 text-sm mb-2">Current Stock</TwText>
            <TwText className="text-white font-bold text-4xl">25</TwText>
          </TwView>

          {/* Type of Adjustment */}
          <TwText className="text-gray-400 text-xs mb-3 font-medium">Adjustment Type</TwText>
          <TwView className="flex-row bg-[#15171E] rounded-xl p-1 mb-6 border border-[#22252D]">
            ...
            >
              <Plus color={adjustmentType === 'add' ? 'white' : '#6B7280'} size={18} className="mr-2" />
              <TwText className={`font-bold text-sm ${adjustmentType === 'add' ? 'text-white' : 'text-gray-400'}`}>Add Stock</TwText>
            </TwPressable>
            ...
            >
              <Minus color={adjustmentType === 'remove' ? 'white' : '#6B7280'} size={18} className="mr-2" />
              <TwText className={`font-bold text-sm ${adjustmentType === 'remove' ? 'text-white' : 'text-gray-400'}`}>Remove Stock</TwText>
            </TwPressable>
          </TwView>

          {/* Form Fields */}
          <TwView className="mb-4">
            <TwText className="text-gray-400 text-xs mb-2 font-medium">Quantity</TwText>
          ...
          <TwView className="mb-4">
            <TwText className="text-gray-400 text-xs mb-2 font-medium">Reason</TwText>
            <TwView className="bg-[#15171E] rounded-xl border border-[#22252D] h-24 flex-row py-3 px-4">
              <TextInput 
                placeholder="Enter reason for adjustment"
                placeholderTextColor="#6B7280"
                style={{ flex: 1, color: 'white', fontSize: 14, textAlignVertical: 'top' }}
                multiline
              />
            </TwView>
          </TwView>
        ...
        <TwView className="absolute bottom-0 w-full px-6 py-4 bg-[#0A0D14] border-t border-[#22252D]">
          <TwPressable className="bg-[#FF6B00] rounded-xl h-12 items-center justify-center w-full">
            <TwText className="text-white font-bold text-base">Update Stock</TwText>
          </TwPressable>
        </TwView>
      </KeyboardAvoidingView>
    </SafeAreaView>
